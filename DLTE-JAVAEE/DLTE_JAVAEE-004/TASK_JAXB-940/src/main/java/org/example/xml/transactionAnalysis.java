...
public class transactionAnalysis {
    public static void main(String[] args) throws JAXBException, FileNotFoundException {
        ...
        List<Transaction> myBank = Stream.of(
                ...
                new Transaction(12575.89, "Vibha", new Date(2024, 8, 21), "emergency")).collect(Collectors.toList());

//        Transaction transaction=new Transaction(myBank);

        MyTransaction myTransaction=new MyTransaction(myBank);
        JAXBContext context=JAXBContext.newInstance(MyTransaction.class);
        Marshaller marshaller = context.createMarshaller();
        marshaller.setProperty(Marshaller.JAXB_FORMATTED_OUTPUT,true);
        marshaller.marshal(myTransaction, new FileOutputStream("Newtransaction.xml"));
        System.out.println("XML has built");
//
//        Unmarshaller unmarshaller=context.createUnmarshaller();
//        Transaction myLists = (Transaction) unmarshaller.unmarshal(new FileInputStream("2024.xml"));
//        myLists.getTransactionTo().forEach(System.out::println);
    }
}
